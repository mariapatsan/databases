...
Внешний ключ resume_users связать поле resume.userid с полем users.id. Выбранные данные В КАСКАДЕ УДАЛЕНИЙ И В КАСКАДЕ ОБНОВЛЕНИЙ.
При удалении пользоватея из пользователи - всѵ связанные с ним резюме в резюме автоматически удаляются. Это избавляет от одиночных записей и обеспечивает ссылочную целостность.
При удалении резюме из резюме - таблѻѸца пользователи не затрагивается, так как каскад настроен только в одну стѾЀоу.
При изменеии id поьзоватея в пользователях - поле userid во всѵх его Ѐзюме автоматически обновляется на нвое значение.
...
Минимум 0 - пользователь может не иметь ни одного резюме. Это возмѼожно, пункттѾѼу что внешний ключ resume.userid - users.id не требует, что у каждогѾ пунктьзоватея были пункт.
