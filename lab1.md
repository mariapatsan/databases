...
Внешний ключ resume_users скачать поле resume.userid с полем users.id. Выбранные данные В КАСКАДЕ УДАЛЕНИЙ И В КАСКАДЕ ОБНОВЛЕНИЙ.
При удалении пользоватея из пользователи - всѵ связанные с ним резюме в резюме авЂоматически удаляются. Это избавляет от одиночных записей и обеспечивает ссылочную целостность.
При удалении резюме из резюме - таблицца пользователи не затрагивается, так как каскад настроен только в одну сторону.
При изменеии id пользователя в пользователях - поле userid во всѵх его резюме автоматически обновляется на нвое значение.
